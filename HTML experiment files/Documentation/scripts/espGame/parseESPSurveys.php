<?php
	...
	$rootPath = "C:/xampp/htdocs/ESP2/experiments/ESP/";
            
	//path to the the firebase json file you downloaded
 	$firebaseJson = "./myExport.json";
	
	//this is where the flattened survey data is saved
 	$outputFile = "./flattenedSurveys.csv";
 ...
 if (file_exists($rootPath . "postGameQuestionnaires/postGameQuestionnaireOrder.json")){
		$surveyOrder = json_decode(file_get_contents($rootPath . "postGameQuestionnaires/postGameQuestionnaireOrder.json"), true);
		foreach($surveyOrder as $surveyFile){
			$surveyData = json_decode(file_get_contents($rootPath . "postGameQuestionnaires/" . $surveyFile['postGameQuestionnaire']), true);
			foreach($surveyData as $surveyItem){
				if($surveyItem['key']){
					array_push($surveyKeys, $surveyItem['key']);
				}
