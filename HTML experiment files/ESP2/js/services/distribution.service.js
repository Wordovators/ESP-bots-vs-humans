...
distributionService.$inject = ['$http','$q','SweetAlert', '$timeout'];
function distributionService($http, $q, SweetAlert, $timeout){
	var distributionService = {};

	distributionService.expId;
	distributionService.filename;

	distributionService.getList = function(expId){
		var def = $q.defer();
	...
	function getTargetList(targetList){
		...
		var ref = firebase.database().ref().child(distributionService.expId).child('lists');
		distributionService.filename = list;
